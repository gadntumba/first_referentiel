...
#[AsCommand(
    name: 'app:rollback-cedrick',
    description: 'Add a short description for your command',
)]
class RollbackCedrickCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $productors = $this->productorRepository->findAll();
        $count = count($productors);

        //$i = 0;
        $i=$firstCol-1;

        for ($i=$firstCol-1; $i < $count; $i++) { 
            $productor = $productors[$i];
            //$line = $dataLines[$i];
        ...
        //foreach ($dataLines as $key => $line) {
            ...
            $oldCoordinates = $productor->getOldCordinates();
            
            if (
                is_null($oldCoordinates) ||
                !isset($oldCoordinates["latitude"]) ||
                !isset($oldCoordinates["longitude"])
            ) 
            {
            ...
            $productor->setLatitude((float) $oldCoordinates["latitude"]);
            $productor->setLongitude((float) $oldCoordinates["longitude"]);
